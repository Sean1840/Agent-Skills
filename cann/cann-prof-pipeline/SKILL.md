...
先验：[overview](../docs/profiling/prior/overview.md)、仓 `prior/*.md`、[error-codes](../docs/profiling/prior/error-codes.md)。细节对不上 dump 就改 `details/`。

工具：`PYTHONPATH` 指到本领域 `tool/`（见 [README](../README.md)），然后 `python -m cann_analyze ...`。
...
同时给了数据+日志时，**先看 `collect`/`evidence` 的 `pid_bind`**（规则见 [analysis-chain.md](../analysis-chain.md)）。`related=false`：停止分析，告知用户 PROF/`info.json` 的 pid 与 plog 对不上，请换同一进程的材料。ascend_pt 被改名则不靠文件名判断 pid。
...
| 判定 | 下一步 |
|---|---|
| 解析 | **必须** `cann-prof-parse`，且按 [analysis-chain.md](../analysis-chain.md) 追到报错那条数据 |
...
分流见先验 overview。全程未到无法定位就继续追数据（[analysis-chain.md](../analysis-chain.md)）。可改代码时再 [version-and-fix.md](../version-and-fix.md)。

## 输出

[report-template.md](../report-template.md)。超出 profiling 链时不要套该模板全文。
